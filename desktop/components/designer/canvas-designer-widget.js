...
function CanvasDesigner() {
    ...
    function syncData(data) {
        ...
        if(designer.syncQueue.length)
        {
            designer.syncQueue.push(data)
        }
        else
        {
            designer.sendSyncData(data);
        }

    }
    designer.sendSyncData=function(data) {
    ...
    function onMessage(event) {
        ...
        if (!!event.data.canvasDesignerSyncDataFinished) {
            if(designer.syncQueue.length) {
                designer.sendSyncData(designer.syncQueue.shift());
            }
            return;
        }
        if (!!event.data.exitDesigner) {
            exitListener();
            return;
        }
    ...
    designer.addReadyListener = function(callback) {
        readyListener = callback;
    };
    designer.addExitListener = function(callback) {
