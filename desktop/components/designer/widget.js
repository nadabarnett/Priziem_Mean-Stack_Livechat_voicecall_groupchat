...
(function() {
    ...
    var is = {
        ...
        set: function(shape) {
            ...
            cache.isLine = cache.isArrow = cache.isArc = cache.isDragLastPath = cache.isDragAllPaths = cache.isRectangle = cache.isEllipse = cache.isTriangle = cache.isStar  = cache.isPolyline = cache.isQuadraticCurve = cache.isBezierCurve = cache.isPencil = cache.isMarker = cache.isEraser = cache.isText = cache.isImage = cache.isHand = false;
            cache['is' + shape] = true;
        }
    };
    ...
    root.classList.add(prefix.lowercase);
    ...
    function loadImage(context,path){
        var img = new Image();
        img.addEventListener("load", function(e) {
            var retryLoadImage=function () {
                if (img.complete == true) {
                    context.drawImage(img, 2, 2, 36, 36);
                } else {
                    setTimeout(retryLoadImage, 50);
                }
            };
            retryLoadImage();
    ...
    function endLastPath() {
        ...
        else if (cache.isPolyline) polylineHandler.end();
        drawHelper.redraw();
    ...
    var drawHelper = {
        ...
        triangle: function(context, point, options) {
            var mx = point[0];
            var my = point[1];
            ...
            var ly = point[3];
            context.beginPath();
            context.moveTo((mx+lx)/2, ly);
            context.lineTo(lx,my);
        ...
        pencil: function(context, point, options) {
            ...
            {
                this.line(context,point[i],options);
                i++;
            }
        },
        eraser: function(context, point, options) {
            var i=0;
            while (i<point.length)
            {
                this.line(context,point[i],options);
        ...
        path: function(context, point, options) {
            var mx = point[0];
            var my = point[1];
            var i=2;
        ...
        star: function(context, point, options) {
            var mx = point[0];
            var my = point[1];
            ...
            var spikes=5;
            var step=Math.PI/5;
            var radiusX=Math.abs(mx-lx)/2;
            var radiusY=Math.abs(my-ly)/2;
            var innerRadius=Math.min(radiusX,radiusY);
            var outerRadius=Math.max(radiusX,radiusY);
        ...
        arc: function(context, point, options) {
            ...
            context.arc(point[0], point[1], Math.abs(point[2]), point[3], 0, point[4]);

            this.handleOptions(context, options);
        },
        ellipse: function(context, point, options) {
            context.beginPath();

            var rx=(point[0]-point[2])/2;
            var ry=(point[1]-point[3])/2;

            context.ellipse(point[0]-rx, point[1]-ry, Math.abs(rx),  Math.abs(ry), 0, 0, 2 * Math.PI);
    ...
    var dragHelper = {
        ...
        mousedown: function(e) {
            ...
            x=(x-zoomHandler.translate[0])/zoomHandler.scale;
            y=(y-zoomHandler.translate[1])/zoomHandler.scale;
            g.prevX = x;
            g.prevY = y;
    ...
    var pencilHandler = {
        ...
        mousedown: function(e) {
            ...
            t.data=[];
            t.data.push([t.prevX, t.prevY, x, y]);
        ...
        mouseup: function(e) {
            var t = this;
            t.ismousedown = false;
            points[points.length] = ['pencil', t.data, pencilDrawHelper.getOptions()];
            t.data=[];
        },
        mousemove: function(e) {
            var x=e.pageX-canvas.offsetLeft,
    ...
    var   markerGlobalAlpha = 1;
    var   markerStrokeStyle = 'rgba(' + hexToRGB(document.getElementById('marker-fill-style').value).join(',') + ',0.4)';
    ...
    var handHandler = {
        ...
        onShapeSelected: function() {
            tempContext.canvas.style.cursor = prefix.css+'grab';
        ...
        mousedown: function(e) {
            tempContext.canvas.style.cursor = prefix.css+'grabbing';
        ...
        mousemove: function(e) {
            ...
            if (t.ismousedown) {
                zoomHandler.applyTranslate((x-t.prevX)/zoomHandler.scale,(y-t.prevY)/zoomHandler.scale);
                t.prevX = x;
                t.prevY = y;
            }
        }
    ...
    var eraserHandler = {
        ...
        mouseup: function(e) {
            var t = this;
            t.ismousedown = false;
            points[points.length] = ['eraser', t.data, drawHelper.getOptions()];
            t.data=[];
        },
        mousemove: function(e) {
            var x = e.pageX - canvas.offsetLeft,
    ...
    var textHandler = {
        ...
        getFillColor: function(color) {
            color = (color || strokeStyle).toLowerCase();
        ...
        fillText: function(text) {
            ...
            drawHelper.handleOptions(tempContext, options, true);
        ...
        mousedown: function(e) {
            ...
            textHandler.x=(textHandler.x-zoomHandler.translate[0])/zoomHandler.scale;
            textHandler.y=(textHandler.y-zoomHandler.translate[1])/zoomHandler.scale;
            ...
            if (g.ismousedown) {
                if (!g.isCircleDrawn && g.isCircledEnded) {
                    ...
                    /*var c = (2 * Math.PI * radius) / 21,
                     ...
                     angle = (xx + yy) / (2 * c);*/
                    var angle=2 * Math.PI;
                    points[points.length] = ['arc', [prevX + radius, prevY + radius, Math.abs(radius), angle, 1], drawHelper.getOptions()];
                    this.end();
    ...
    var triangleHandler = {
        ...
        mouseup: function(e) {
            ...
            var t = this;
            if (t.ismousedown) {
                points[points.length] = ['triangle', [t.prevX, t.prevY, x, y], drawHelper.getOptions()];
        ...
        mousemove: function(e) {
            ...
            if (t.ismousedown) {
                ...
                drawHelper.triangle(tempContext, [t.prevX, t.prevY, x, y]);
            }
        }
    };
    ...
    var starHandler = {
        ...
        mouseup: function(e) {
            ...
            var t = this;
            if (t.ismousedown) {
                points[points.length] = ['star', [t.prevX, t.prevY, x, y], drawHelper.getOptions()];
        ...
        mousemove: function(e) {
            ...
            if (t.ismousedown) {
                ...
                drawHelper.star(tempContext, [t.prevX, t.prevY, x, y]);
            }
        }
    };
    var rectHandler = {
    ...
    var ellipseHandler = {
        ...
        mouseup: function(e) {
            ...
            var t = this;
            if (t.ismousedown) {
                points[points.length] = ['ellipse', [t.prevX, t.prevY, x, y], drawHelper.getOptions()];
        ...
        mousemove: function(e) {
            ...
            if (t.ismousedown) {
                ...
                drawHelper.ellipse(tempContext, [t.prevX, t.prevY, x, y]);
            }
        }
    };
    ...
    var polylineHandler = {
        ...
        mousemove: function(e) {
            ...
            g.data[g.index*2]=x;
            g.data[g.index*2+1]=y;
            drawHelper.path(tempContext, g.data);
        },
            ...
            if(g.data.length)
            {
                points[points.length] = ['path', g.data, drawHelper.getOptions()];
                syncPoints(is.isDragAllPaths || is.isDragLastPath ? true : false);
            }
            g.data=[];
            g.index=0;
    ...
    triangleHandler=clone(polylineHandler);
    triangleHandler.mouseup=function(e) {
        var g = this.global;
        g.ismousedown = false;
        if(g.index==3)
        {
    ...
    var zoomHandler = {
        ...
        apply:function(){
            ...
            tempContext.translate(this.translate[0], this.translate[1]);
            tempContext.scale(this.scale, this.scale);

            transformGrid(this.scale, this.translate);

            drawHelper.redraw();

        },
        applyTranslate:function(dx,dy){
            var dxScaled=dx*this.scale;
            var dyScaled=dy*this.scale;
            var newTx=this.translate[0]+dxScaled;
            var newTy=this.translate[1]+dyScaled;
            var maxTx=innerWidth*(this.scale-1);
            var maxTy=innerHeight*(this.scale-1);

            if(newTx<-maxTx)
            {
                newTx=-maxTx;
                dx=(newTx-this.translate[0])/this.scale;
            }
            if(newTx>0)
            {
                newTx=0;
            ...
            {
                newTy=-maxTy;
                dy=(newTy-this.translate[1])/this.scale;
            }
            if(newTy>0)
            {
                newTy=0;
            ...
            this.translate[0]=newTx;
            this.translate[1]=newTy;
            context.translate(dx, dy);
            tempContext.translate(dx, dy);
            transformGrid(this.scale, this.translate);
            drawHelper.redraw();
        },
        zoom: function(direction) {
            var factor = 0.1,
                ...
                translate0 = [],
                l = [],
                view = {x: translate[0], y: translate[1], k: this.scale};

            target_zoom = this.scale * (1 + factor * direction);

            if (target_zoom < extent[0])
            {
                if(extent[0]==this.scale)
                {
                    return false;
                }
                else
            ...
            //console.log(this.scale,">",target_zoom);
            ...
            var maxTx=innerWidth*(view.k-1);
            var maxTy=innerHeight*(view.k-1);
            if(view.x<-maxTx)
            {
                view.x=-maxTx;
            }
            ...
            return d3.transition().duration(350).tween("zoom", function () {
                var iTranslate = d3.interpolate(_self.translate, [view.x, view.y]),
                    iScale = d3.interpolate(_self.scale, view.k);
                return function (t) {
                    _self.scale=iScale(t);
                    _self.translate=iTranslate(t);
                    _self.apply();
                };
            });
    ...
    var FileSelector = function(accept='image/*') {
        var selector = this;
    ...
    (function() {
        ...
        function bindEvent(context, shape) {
            ...
            addEvent(context.canvas, 'click', function() {
                ...
                if (shape === 'Text') {
                    ...
                    textHandler.onShapeUnSelected();
                    if (shape === 'Hand') {
                        handHandler.onShapeSelected();
                    } else {
                        handHandler.onShapeUnSelected();
                    }
                }
        ...
        function decorateEllipse() {
            var context = getContext('ellipse');

            context.ellipse(20, 20, 18,14,0, 0,  Math.PI * 2);
            context.stroke();

            context.fillStyle = 'Gray';
            context.font = '9px Verdana';
            context.fillText('Ellipse', 6, 24);

            bindEvent(context, 'Ellipse');
        }

        if (tools.ellipse === true) {
            decorateEllipse();
        } else document.getElementById('ellipse').style.display = 'none';

        function decoratePolyline() {
            var context = getContext('polyline');
            var x = 13;
            var y = 3;
            ...
            context.fillText('Polyline', 5, 24);

            bindEvent(context, 'Polyline');
        }

        if (tools.polyline === true) {
            decoratePolyline();
        } else document.getElementById('polyline').style.display = 'none';

        function decorateTriangle() {
            var context = getContext('triangle');
            var x = 20;
            var y = 1;

            context.beginPath();
            context.moveTo(x, y);
            context.lineTo(x-19,y+36);
            context.lineTo(x+19,y+36);
            context.closePath();
            context.stroke();
            ...
            context.fillText('Triangle', 5, 35);

            bindEvent(context, 'Triangle');
        }

        if (tools.triangle === true) {
            decorateTriangle();
        } else document.getElementById('triangle').style.display = 'none';

        function decorateStar() {
            var context = getContext('star');
            ...
            context.fillStyle = 'Gray';
            context.font = '9px Verdana';
            context.fillText('Star', 10, 24);

            bindEvent(context, 'Star');
        }
        ...
        function decorateHandTool() {
            var context = getContext('hand-tool');

            loadImage(context,"./../../css/svg/hand.svg");
            canvas = context.canvas;
            bindEvent(context, 'Hand');
        ...
        function decorateClearAllTool() {
            var context = getContext('clear-tool');


            loadImage(context,"./../../css/svg/clean.svg");
            ...
            addEvent(canvas, 'click', function() {
                ...
                if (confirm('Are you sure you want to clear the whiteboard?')) {
                    processUndo({uid:uid,undo:true,index:"all"})
                } else {
                }
            });
        }
        decorateClearAllTool();

        function decorateUndoTool() {
            var context = getContext('undo-tool');

            loadImage(context,"./../../css/svg/undo-arrow.svg");

            canvas = context.canvas;

            addEvent(canvas, 'click', function() {
                hideContainers();
                processUndo({uid:uid,undo:true,index:-1})
            });
        }
        decorateUndoTool();

        function decorateRedoTool() {
            var context = getContext('redo-tool');

            loadImage(context,"./../../css/svg/redo-arrow.svg");

            canvas = context.canvas;

            addEvent(canvas, 'click', function() {
                hideContainers();
                processRedo({uid:uid,redo:true})
            });
        }
        decorateRedoTool();

        function decorateGridTool() {
            var context = getContext('grid-tool');

            loadImage(context,"./../../css/svg/grid.svg");
        ...
        function decorateLineWidth() {
            ...
            var lineWidthContainer = find('line-width-container'),
                ...
                lineWidthList = find('line-width-list'),
                h1 = document.getElementsByTagName('h1')[0],
                canvas = context.canvas;

            var cwClose=document.createElement("div");
            cwClose.setAttribute('class',"modal-close-mini");
            cwClose.innerHTML="&times;";
            lineWidthContainer.appendChild(cwClose);
            cwClose.onclick = function(event) {
                hideContainers();
            };

            var lineSizes = [1,2,4,6,8];

            lineSizes.forEach(function(lineSize) {
                var row = '<tr>';
                row += '<td data-size="' + lineSize + '"><hr noshade title="' + lineSize + '" size="' + lineSize + '"/></td>';
                row += '</tr>';
                lineWidthList.innerHTML += row;
            });

            // console.log(markerColorsList.getElementsByTagName('td'))
            Array.prototype.slice.call(lineWidthList.getElementsByTagName('td')).forEach(function(td) {
                addEvent(td, 'click', function() {
                    pencilLineWidth=lineWidth = td.getAttribute('data-size');
                    hideContainers();
            ...
            addEvent(lineWidthText, 'input', function() {
                pencilLineWidth=lineWidth = lineWidthText.value;
            });
            addEvent(canvas, 'click', function() {
                hideContainers();
        ...
        function decorateColors() {
            ...
            var colorsContainer = find('colors-container'),
                ...
                canvas = context.canvas;
            addEvent(strokeStyleText, 'change', function() {
                pencilStrokeStyle = strokeStyle = strokeStyleText.value;
            });
            addEvent(fillStyleText, 'change', function() {
                fillStyle = fillStyleText.value;
            });
            addEvent(strokeStyleText, 'click', function(event) {
                event.preventDefault();
                event.stopImmediatePropagation();
            });
            addEvent(fillStyleText, 'click', function(event) {
                event.preventDefault();
                event.stopImmediatePropagation();
            });
            addEvent(canvas, 'click', function() {
        ...
        var lastStrokeColors=['ffffff', 'ffffff', 'ffffff', 'ffffff', 'ffffff'];
        var lastFillColors=['ffffff', 'ffffff', 'ffffff', 'ffffff', 'ffffff'];
        var basicColors=[['80ff00', '00ff01', '00ff7f', '00ffff', '0080ff', '0000fe'],
            ['7f00ff', 'ff00fe', 'ff017e', 'fe0000', 'ff7f00', 'ffff00']];


        function decorateColorInputs() {
            ...
            var cw = Raphael.colorwheel(document.getElementById('colorPickerContent'), 400);
            var cwClose=document.createElement("div");
            cwClose.setAttribute('class',"modal-close");
            cwClose.innerHTML="&times;";
            document.getElementById('colorPickerContent').appendChild(cwClose);
            var colorPickerBox=document.getElementById('colorPicker');
            window.onclick = function(event) {
                if(event.target==colorPickerBox)
                {
                    cwClose.onclick();
                }
            };
            cw.onchange(function(){
                if(currentStyleText)
                {
                    currentStyleText.value=cw.color();
                    currentStyleText.data=currentStyleText.value;
                    if(currentStyleText.parentNode.classList.contains('transparentInput'))
                    {
                        currentStyleText.parentNode.classList.toggle("transparentInput");
                    }
                    currentStyleText.dispatchEvent(new Event("change"));
                }

            });
            cwClose.onclick = function(event) {
                colorPickerBox.style.display = "none";
                if(lastColors && currentStyleText && currentStyleText.data)
                {
                    var lastColor=currentStyleText.data.replace('#','').toLowerCase();
                    if(lastColor!='transparent' && lastColors.indexOf(lastColor)==-1)
                    {
                        lastColors.pop();
                        lastColors.unshift(lastColor);
                    }
                    lastColors=null;
                }
            ...
            addEvent(strokeStyleText, 'change', function() {
                pencilStrokeStyle = strokeStyle = strokeStyleText.data;
            });
            addEvent(fillStyleText, 'change', function() {
                fillStyle = fillStyleText.data;
            });

            var processColorClick=function(event)
            {
                event.preventDefault();
                event.stopImmediatePropagation();
                colorPickerBox.style.display = 'block';
                cw.color(strokeStyle);
                var colorsList=document.getElementById('colors-list');
                var colorRows=[];
                colorRows.push(lastColors);
                for(var i=0;i<basicColors.length;i++)
                {
                    colorRows.push(basicColors[i]);
                }
                colorsList.innerHTML = "";

                colorRows.forEach(function(colorRow) {
                    var row = '<tr>';
                    if(colorRow.length==5)
                    {
                        row+= '<td class="transparentPicker" data-color="transparent"></td>';
                    ...
                    colorsList.innerHTML += row;
                });
                Array.prototype.slice.call(colorsList.getElementsByTagName('td')).forEach(function(td) {
                    addEvent(td, 'click', function() {
                        var elColor = td.getAttribute('data-color');
                        var _color = (elColor=='transparent'?elColor:('#' + elColor));

                        cw.color(_color=='transparent'?"#ffffff":_color);
                        currentStyleText.value=cw.color();
                        currentStyleText.data=(_color=='transparent'?'transparent':currentStyleText.value);
                        currentStyleText.dispatchEvent(new Event("change"));

                        if(currentStyleText.parentNode.classList.contains('transparentInput') != (_color=='transparent'))
                        {
                            currentStyleText.parentNode.classList.toggle("transparentInput");
                        }
            ...
            addEvent(strokeStyleText, 'click', function(event) {
                ...
                currentStyleText=strokeStyleText;
                lastColors=lastStrokeColors;
                cw.color(strokeStyle=='transparent'?"#ffffff":strokeStyle);
                processColorClick(event);

            });
            addEvent(fillStyleText, 'click', function(event) {
                currentStyleText=fillStyleText;
                lastColors=lastFillColors;
                cw.color(fillStyle=='transparent'?"#ffffff":fillStyle);
                processColorClick(event);
            });
        }
        decorateColorInputs();
        ...
        function decorateButton(name, label,handler) {
            var context = getContext(name);
            canvas = context.canvas;

            context.fillStyle = "#0C8FB3";
            context.font = '15px Verdana';
            var m=context.measureText(label);
            context.fillText(label, (canvas.width-m.width)/2, 20);
        ...
        decorateButton('exitButton','Exit',exitDesigner);
        decorateButton('saveButton','Save',saveDesigner);
        decorateButton('openButton','Open',openDesigner);
    ...
    addEvent(canvas, isTouch ? 'touchstart mousedown' : 'mousedown', function(e) {
        ...
        else if (cache.isHand) handHandler.mousedown(e);
        else if (cache.isTriangle) triangleHandler.mousedown(e);
        else if (cache.isStar) starHandler.mousedown(e);
        else if (cache.isPolyline) polylineHandler.mousedown(e);
        else if (cache.isEllipse) ellipseHandler.mousedown(e);

        if((!cache.isPolyline || polylineHandler.global.index==0) && (!cache.isTriangle || triangleHandler.global.index==0))
        {
            drawHelper.redraw();
    ...
    addEvent(canvas, isTouch ? 'touchend touchcancel mouseup' : 'mouseup', function(e) {
        ...
        else if (cache.isHand) handHandler.mouseup(e);
        else if (cache.isTriangle) triangleHandler.mouseup(e);
        else if (cache.isStar) starHandler.mouseup(e);
        else if (cache.isPolyline) polylineHandler.mouseup(e);
        else if (cache.isEllipse) ellipseHandler.mouseup(e);

        if(!cache.isPolyline || polylineHandler.global.index==0)
        {
            drawHelper.redraw();
            syncPoints(is.isDragAllPaths || is.isDragLastPath ? true : false);
        }
    ...
    addEvent(canvas, isTouch ? 'touchmove mousemove' : 'mousemove', function(e) {
        ...
        else if (cache.isHand) handHandler.mousemove(e);
        else if (cache.isTriangle) triangleHandler.mousemove(e);
        else if (cache.isStar) starHandler.mousemove(e);
        else if (cache.isPolyline) polylineHandler.mousemove(e);
        else if (cache.isEllipse) ellipseHandler.mousemove(e);

        preventStopEvent(e);
    });
    addEvent(canvas, 'dblclick', function(e) {
        var cache = is;
        if (cache.isPolyline) polylineHandler.end(true);
    });
    var keyCode;
    ...
    function onkeydown(e) {
        ...
        if (cache.isPolyline && e.keyCode === 27)
        {
            polylineHandler.end();
        }
    }
    ...
    var uid;
    var redoPoints=[];
    var processRedo=function(data){
        points.push(redoPoints.pop());
        drawHelper.redraw();
        if(!data.noSync)
        {
            syncPoints(true,"redo");
        }
    };
    var processUndo=function(data){
    ...
    window.addEventListener('message', function(event) {
        ...
        if (!uid) {
            uid = event.data.uid;
            console.log('designer',uid,event);
            isReadyDesigner();
            console.log('designer has been set')
        ...
        if (event.data.redo && redoPoints.length) {
            processRedo(event.data);
            return;
        }
        if (event.data.undo && points.length) {
            processUndo(event.data);
            return;
        }
        ...
        if(d.type=="undo")
        {
            processUndo({uid:uid,undo:true,index:-1, noSync:true});
        }
        else if(d.type=="redo")
        {
            processRedo({uid:uid,redo:true, noSync:true});
        }
        else if(d.type=="undo-all")
        {
            processUndo({uid:uid,undo:true,index:"all", noSync:true})

        }
        else if (d.startIndex !== 0) {
        ...
        window.parent.postMessage({
            canvasDesignerSyncDataFinished: true,
            uid: uid
        }, '*');

    }, false);

    function syncPoints(isSyncAll, type) {
        if (isSyncAll) {
            lastPointIndex = 0;
        }

        if (!isSyncAll && lastPointIndex == points.length) return;

        var pointsToShare = [];
        var i=0;
        if(!type)
        {
            for (i = lastPointIndex; i < points.length; i++) {
                if(points[i][0]!='text' && points[i][2] && points[i][2][7])
                {
                    delete points[i][2][7];
                }
                pointsToShare[i - lastPointIndex] = points[i];
                if(pointsToShare.length>LINES_LIMIT)
                {
                    break;
                }
            }
        ...
        if (isSyncAll || pointsToShare.length) {
            syncData({
            ...
            console.log( (pointsToShare || []).length);
        }
    ...
    function saveDesigner() {
        ...
        saveAs(blob, "PrizeWhiteboard_"+datetime+'.pzwb');
    }

    var openFileHandler=function(file) {
        if (!file) return;

        var reader = new FileReader();
        reader.onload = function(event) {
            try {
                var _points=JSON.parse(event.target.result)
            }
            catch (err)
            {
                console.log(err);
    ...
    function openDesigner() {
        var selector = new FileSelector('.pzwb');
        selector.selectSingleFile(openFileHandler);
    }
    ...
    function fileSelectHandler(e) {
        ...
        if(controlGroup && controlGroup.classList && !controlGroup.classList.contains('whiteboard-control-group-invisible')) {
            fileDragHover(e);
    ...
    function fileDragHover(e) {
        ...
        if(controlGroup && controlGroup.classList && !controlGroup.classList.contains('whiteboard-control-group-invisible')) {
            if (e.type == "dragover") {
                if (!e.target.classList.contains('drag-over-color')) {
                    e.target.classList.add('drag-over-color');
                ...
                if (e.target.classList.contains('drag-over-color')) {
                    e.target.classList.remove('drag-over-color');
